...
class TaskRegistry:
    ...
    def _load_manifest(self, path: Path) -> TaskSpec:
        ...
        return TaskSpec(
            ...
            model_family=data.get("model_family", "LLM"),
            stage=data.get("stage", "SFT"),
            template=data.get("template", ""),
            attn_implementation=data.get("attn_implementation", ""),
            freeze_config=data.get("freeze_config", ""),
            max_seq_len=data.get("max_seq_len", 8192),
            dataset_type=data.get("dataset_type", "erniekit"),
            model_defaults=data.get("model_defaults", {}),
            finetuning_defaults=data.get("finetuning_defaults", {}),
